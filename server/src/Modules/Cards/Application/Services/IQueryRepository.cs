...
namespace Cards.Application.Services
{
    ...
    {
        Task<IEnumerable<Repeat>> GetRepeats(UserId userId, DateTime dateTime, int count, int questionLanguage, CancellationToken cancellationToken);
        Task<int> GetDailyRepeatsCount(UserId userId, DateTime dateTime, int questionLanguage, CancellationToken cancellationToken);
        Task<int> GetNewRepeatsCount(UserId userId, int questionLanguage, Guid? groupId, CancellationToken cancellationToken);
